...
class UserController:
    ...
    def login():
        ...
        if "cookie" in body and "id" in body:
            ...
            if (token and 
                ...
                cookie = reissue_token(user.id,7) #cookie expires after 7 days of no login
        ...
        if user and match_hash(user.password,password):
            ...
            if existingToken: invalidate_token(existingToken)
            if remember:
                cookie = reissue_token(user.id,7) #cookie expires after 7 days of no login
            else: cookie = reissue_token(user.id,1) # cookie expires after 1 day
            return jsonify({"Message":"Login Successful","cookie":cookie,"id":user.id}), 200
    ...
    def logout():
        ...
        try:
            ...
            if user.loginToken: invalidate_token(user.loginToken)
    ...
    def home(user):
        ...
        for r in user.recipes:
            ...
            ingredients = []
            ingredientLinks = r.ingredientLinks # from linker between ingredients and recipes
            ...
            for link in ingredientLinks:
                ...
                ingredient = IngredientNames.query.filter_by(id=ingredientNameId).first()
                measurement = Measurements.query.filter_by(id=measurementId).first()
    ...
    def new_recipe(user):
        ...
        try:
            name = body["name"]
            ingredients = body["ingredients"] # array of objects with name, whole, numerator, denominator, unit
            meals = body["meals"]
        ...
        recipe = Recipes(name=name,ownerId=user.id,instructions=instructions,uri=uri)
        ...
        for m in meals:
            ...
            try: db.session.add(MealsRecipes(mealId=meal.id,recipeId=recipe.id))
            except: return jsonify({"Message":"An Error Occured"}), 500
        ...
        for ingredient in ingredients:
            ...
            measurement = add_to_db(Measurements,{"whole":whole,"numerator":numerator,"denominator":denominator,"unit":unit})
            ingredient=add_to_db(Ingredients, {"measurementId":measurement.id,"ingredientNameId":ingredientName.id})

            newRecipeIngredients = RecipesIngredients(recipeId=recipe.id, ingredientId=ingredient.id)
